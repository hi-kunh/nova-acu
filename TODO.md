...
- **▶ 화요일(9/15) 여기서 바로 시작** (2026-09-14 월 진행 기록)
  ...
  **3. ✅ 사용자 저장 `users.db` — 9/14에 앞당겨 끝냄** (상세는 README 같은 날짜 절)
  ...
  - 실측(`tools/users_bench.c`): **보드에서 26만 명 다운로드 65.6초**(9/11 Python 추정 1시간 → 65초),
    카드 조회 **0.0808ms**, 개별 등록 0.166ms, 파일 47MB
  ...
  **4. 🙏 DM에 추가로 요청할 것 (9/14 오후 추가)**
  - [ ] **사용자 레코드 원시 바이트 캡처** — `UserSend`(1명) 또는 `UserBinTransSendStart/Continue`(일괄).
        AccessOption 비트 배치(특히 **Enable**)와 Proximity Data 32byte의 실제 형식을 확인해야 한다.
        `dm/README.md` "진행 중"에 추가하고, 다음 회신에 함께 넣는다
...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  | ~~**9/16 (수)**~~ ✅ | **사용자 저장 `users.db`** — 규약 필드 스키마, 묶음 커밋, 새 표에 받아 교체 | **9/14에 앞당겨 완료** — 보드 26만 명 65.6초 / 조회 0.0808ms.<br>남은 것: `6. FileBinaryTransmit` 문서 읽기 + **수신 프로토콜 구현**(UserBinTrans·UserSend) |

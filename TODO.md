...
- 최종 갱신: 2026-09-07
- 현재 위치: **5단계 완료 / 6단계 설계 확정 중** (하드웨어 구성·링크·MCU 확정, 프로토콜 설계 남음)
...
> **다음 작업 시작 지점**: Wiegand 인터럽트 핀 확인 (아래 첫 항목). 이게 최종 RRU 보드 구성을 가른다.

- [ ] **[최우선] Wiegand 수신용 외부 인터럽트 핀 확인 후 방식 확정** — 리더 8개 x D0/D1 = 16개 필요한데
      STM32 EXTI는 통상 16라인이고 핀 번호를 포트끼리 공유해 여유가 없다. 확인 항목과 선택지(A~D)는
      HARDWARE.md "주의 3" 참고. 유력안은 **리더당 게이트로 묶어 인터럽트 8개로 줄이기**
...
- [ ] **최종 RRU 보드 I/O 설계** — LQFP64(`RE`)로는 48 I/O(Wiegand16+입력24+출력8)가 한계 초과.
      MCU는 STM32C562로 고정하되 **패키지는 재검토**: 계열 내 상위 핀수 옵션 확인,
      Wiegand D0/D1 다이오드 OR(리더당 EXTI 1개), 입출력 익스팬더 등

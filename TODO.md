...
- **▶ 화요일(9/15) 여기서 바로 시작** (2026-09-14 월 진행 기록)
  ...
  **1-2. ⏸ DM 회신 대기 — C부류(조회 계열) 전수 점검 결과** (2026-09-14 접수)
  - `dm/NCU_통보_미구현명령_2종.md` — 장치구조 반영은 🟢 확인됐고, **실패한 명령 2종**이 왔다
    - `SettingControllerBasisValueCheck` (**6/2/41**) · `SettingInputValueCheck` (**6/2/44**) — 둘 다 acud 미구현
    - Platinum 「장치 정보 업데이트」는 `*Check`를 한꺼번에 보내서 **하나만 실패해도 화면 전체가 실패**로 보인다
  - DM이 **C부류 전수 점검 중**이고 끝나면 "현장 SSC-324가 답하는 명령 전체 목록"을 준다.
    **그 목록이 구현할 조회 명령의 확정 목록**이므로 2종만 따로 치지 않고 **목록을 받아 한 번에 회신·구현**한다 (사용자 지시)
  - [ ] 목록이 오면 → `dm/README.md` "⏸ 회신 보류 중" 4개 항목으로 회신 문서 작성
        (특히 ⚠ **DM의 "입력 설정 쓰기 7건 성공"은 사실과 다르다** — acud에 Object 44 핸들러가 아예 없다)
  - [ ] 그 뒤 구현 — 9/17(목) DM 명령 작업과 묶는 것이 자연스럽다
...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  | **9/17 (목)** | **DM 명령·이벤트** — 강제 개방 Object 206 (변경·확인), 화재·알람 이벤트(입력 용도 → 코드 규칙), USB 단절 이벤트 골격. **+ C부류 목록이 왔으면 조회 명령 2종 이상(Object 41·44)** | idti_client로 206 명령 확인, DM 화면에 화재·알람 이벤트 표시, Platinum 「장치 정보 업데이트」가 실패로 안 뜸 |

...
- [ ] **UDP 브로드캐스트 탐색/설정 구현 (netmodule 프로토콜 호환)** — 우선순위 높음.
      UDP 1460/5001, `FIND`/`IMIN`(50byte)/`SETT`(58byte). 프레임은 README "netmodule UDP
      탐색/설정 프로토콜" 참고. **구현하면 기존 IntelliScan Device Manager가 PC 쪽 변경 없이
      우리 ACU를 발견·설정한다.** webui의 닭-달걀 문제(접속해야 고치는데 접속이 안 됨)를 없앤다
  - [ ] Serial 계열 필드는 시리얼-이더넷 모듈 시절 잔재 — 고정값으로 채우고 수신 시 무시
  - [ ] `SETT` 수신 시 `deploy/acu-netcfg`를 그대로 호출할지(충돌검사/롤백 재사용) 설계
  - [ ] 비밀번호: Company `"IDTi"` 고정 + Custom 기본 `"0000"`

...
- **다음 시작 지점** (2026-09-10 마감 기준)
  ...
  0. **RRU 여러 대 구성 (2026-09-11)** — **최대 대수 결정이 먼저**: V2 모듈 14개 한계 vs 요구 16대.
     HARDWARE.md "RRU 여러 대 구성" 절
...
> 요구 최대 16대 정도. 아래 항목보다 먼저 볼 것 — 상세·근거는 [HARDWARE.md](HARDWARE.md) "RRU 여러 대 구성"
>
> - [ ] **최대 RRU 대수 결정** — V2 모듈 14개 한계(기존 장비와 같음: 리더 28) vs 16대 요구(모듈2만 써도 모듈 32 → V3 필요,
>       대체 원칙과 충돌). DM 쪽 확인 필요
...
> - [ ] USB 허브 구성 (보드 포트 4개 → 추가 허브), RRU 식별 프로토콜(DIP ID + **모델명** + 펌웨어 버전), 모델명→구성표, 모듈 번호 배정 규칙
> - [ ] **RRU 모델명 정하기** (모듈2 / 모듈4) — 사용자 결정

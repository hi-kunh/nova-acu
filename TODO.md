...
- **▶ 월요일(9/14) 여기서 바로 시작** (2026-09-11 금 마감 기록)

  **1. 아침 점검 (10분)**
  - [ ] 보드 전원 켜기 (금요일에 끔) → `ssh rock@192.168.0.250` → `systemctl is-active acud` 가 `active`
  - [ ] `git pull` 후 **DM 추가 회신(실측 결과)** 이 왔는지 확인 (`ls dm/`, `git status`) —
        질의 1~7 회신은 **9/11 오후 도착·반영 끝** (`dm/NCU_DM_질의_RRU구성_이벤트보관_회신.md`, HARDWARE.md "DM 구현 규칙")
  - [ ] (사용자) **DM에서 ncu01 "장치구조 다운로드"** — 서버는 아직 모듈 4개다(폴링으로는 안 바뀜).
        아래 1-1의 mType 수정 배포 **뒤에** 누르면 한 번에 끝난다. 모듈 2개(리더 4 · 입력 16 · 출력 8)인지

  **1-1. 먼저 작은 수정 (DM 회신 반영, 30분~1시간)**
  - [ ] **카드 이벤트 주소를 1부터** — 지금 `(0,0)` (`net.c` `build_event_info`) → 모듈 1, 리더 1
  - [ ] **모듈 종류 기본값 `mType 61`, `installedtype 1`** (`config.c` `ACU_DEFAULT_MODULE_TYPE`/`_INSTALL_TYPE`) — 현장 SSC-324와 같게
  - [ ] 보드 배포 → 사용자가 DM 장치구조 다운로드 → 서버 표 `11000000000000` 확인
  - [ ] **DM에 실측 요청 전달** — 목록은 **`dm/README.md` "진행 중"** 에 이미 적어 둠 (특히 **출력 설정 명령 형식**).
        DM 문서는 앞으로 전부 `dm/` 폴더 — 질의 `NCU_<주제>_질의.md`, 회신은 같은 이름 + `_회신`

  **2. 오늘 작업: acud 이벤트 구동 구조** — 지금 구조와 바꿀 곳
  - 지금: `main.c`가 `CARD_POLL_INTERVAL_MS 2000`마다 `hal_read_card()`를 부르고, 그 사이 `net_poll()`의 select가
    TCP와 탐색 소켓(`net_set_aux_reader`, **fd 1개만** 등록 가능)을 기다린다 → 카드가 최대 2초 늦게 처리된다
  - [ ] **select를 main 루프 하나로 모은다** — fd와 콜백을 여러 개 등록하는 작은 루프 모듈(예: `loop.c`).
        net(리슨·클라이언트), 탐색 소켓, 카드 입력이 각각 fd를 등록한다. `net_set_aux_reader` 대체
  - [ ] **HAL에 카드 입력 fd를 둔다** — mock은 timerfd 등으로 더미 카드를 "도착"시키고, 6단계에서 RRU별 USB fd로 바뀐다.
        **RRU 최대 7대 자리**(RRU 번호 1~7)를 미리 배열로
  - [ ] `CARD_POLL_INTERVAL_MS`·`timespec` 보조 함수 제거, 문 센서 읽기도 이벤트/타이머로
  - [ ] 확장을 막지 않게: 모듈 구성은 지금처럼 설정값, RRU 연결 시 바꿀 수 있는 자리만
  - **끝 기준**: mock 카드가 들어오는 즉시 판정·이벤트 적재(로그 시각으로 확인), DM 폴링·탐색 정상, 보드 배포
...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  | 날짜 | 작업 | 끝났다고 보는 기준 |
  |------|------|------|
  | **9/14 (월)** | DM 회신 확인·반영 → **acud 이벤트 구동 구조** (select로 fd 대기, 2초 주기 조회 제거, RRU 여러 대를 받을 자리) | mock HAL로 카드 이벤트 즉시 처리, 보드 배포 |
  ...
  | **9/17 (목)** | **DM 명령·이벤트** — 강제 개방 Object 206 (변경·확인), 화재·알람 이벤트(입력 용도 → 코드 규칙), USB 단절 이벤트 골격 | idti_client로 206 명령 확인, DM 화면에 화재·알람 이벤트 표시 |

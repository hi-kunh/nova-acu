...
- 최종 갱신: 2026-09-15 (화) 저녁 — 오후 계획 3건 + 9/16 1번 완료, **9/16은 2번(TODO 정리)부터**
- 이전 갱신: 2026-09-14 (월) — **이번 주 예정 작업(9/15~9/18)을 하루에 모두 끝냄**
- 현재 위치: **5단계 완료 / 5.5단계(ACU 단독 개통) 완료 — 실제 DM 연동까지 검증.** RRU 개발보드 도착 전 작업
- **▶ 일정 (2026-09-15 화 오후 ~ 09-18 금)** — 9/15 오후에 **실측 작업량 기준으로 다시 세움**

  **왜 다시 세웠나** — 9/14에 세운 "한 주" 계획을 9/14·9/15 이틀에 다 해 버렸다. 계획이 실제를 못 따라가면
  일정표가 쓸모없어진다. 그래서 **지난 이틀에 실제로 걸린 시간**으로 단위를 다시 잡았다.

  | 일의 종류 | 실측 (9/14~15) |
  |---|---|
  | 프로토콜 기능 하나 (구현 + 시험 + 보드 배포) | **30~60분** — 예: 설정 조회 3종 40분, 이벤트 보관 3종 + 자동 복구 50분 |
  | 실제 DM으로 확인 한 번 (사용자 조작 포함) | **15~30분** |
  | DM 회신 읽고 반영 + 회신 작성 | **30~60분** |
  | 설계 문서 한 편 | **40~60분** |

  **계획 규칙**
  - 하루(8시간) 중 **약 70%만 일을 채운다.** 나머지 30%는 **회신 반영과 예상 못 한 문제** 몫이다
    (9/15에만 카드리더 0byte, 자동 복구 결함, 시험 코드 결함 3건이 끼어들었다)
  - **실제 DM 확인은 사용자 조작이 필요하다** — 날마다 "사용자에게 부탁할 것"을 따로 적는다
  - ⚠ 빨랐던 이유는 **개발 PC + mock에서 끝나는 프로토콜 작업**이었기 때문이다.
    RRU 보드·OS 자체 빌드처럼 **하드웨어가 끼는 일은 이 속도가 안 난다** — 착수할 때 따로 단위를 잡는다
  ...
  **9/15 (화) 오후 — 약 2시간**
  ...
  | # | 작업 | 시간 | 끝 기준 |
  |---|---|---:|---|
  | 1 | ✅ **시험 한 번에 돌리기 `tools/run_tests.sh`** (완료 — PC 9/9 14초, 보드 6/6 19초) — 로컬·보드 겸용. acud를 띄우고 시험 7종을 돌리고 정리까지.<br>오늘 시험 돌리는 데 시간이 많이 들었고 `pkill -f`가 자기 셸을 죽이는 사고도 났다 | 30분 | 명령 하나로 전부 통과/실패 요약 |
  | 2 | ✅ **실제 DM 사용자 다운로드 끝까지** (완료 — DM이 `0x22`로 67명, 카드 `0712BD22` 태그 → DM 화면에 「최웅 부영사」) — 사용자 경로(일괄·1명씩)는 **시험기로만** 확인했고 **실제 DM으로는 한 번도 안 해 봤다.** 가장 큰 미검증 | 50분 | DM에서 받은 사용자의 카드로 태그 → DM 화면에 **그 사용자 이름**으로 출입 이벤트 |
  | 3 | ✅ **알람 릴레이 실제 DM 확인** (완료 — Alarm 두 칸만 동작, Door 그대로, DM 화면에 알람·화재 4건) — DM에서 출력 하나를 동작 종류 Alarm으로 설정 → mock 알람 입력 | 20분 | 그 출력만 동작(로그) + DM 화면에 알람 이벤트 |
  | 4 | 여유 · 기록 · push | 20분 | |

  🙋 **사용자에게 부탁할 것**: (2) DM에서 ncu01로 **카드 사용자 2~3명 다운로드** · (3) 출력 한 칸을 **동작 종류 Alarm**으로 설정
  ⚠ 2에서 문제가 나오면(카드 byte 순서, AccessOption 등) 고치는 데 시간을 쓰고 **3은 수요일로 넘긴다**

  ---

  **9/16 (수)** — 정리하고 사용자 조회 명령

  | # | 작업 | 시간 | 끝 기준 |
  |---|---|---:|---|
  | 1 | ✅ 아침 점검 + **DM 회신 반영** — **9/15 저녁에 앞당겨 끝냄**: Blocking(1/100건)·ReRequestEvent·Event Count 헤더, 카드리더 쓰기 실제 DM 성공, 회신 push. 9/16 아침은 `git -C dm pull`로 새 회신만 본다 | 1시간 → 20분 | 회신이 없으면 바로 2번 |
  | 2 | **TODO.md 정리** — 끝난 일이 미완료로 남아 있다(화재·알람 이벤트, 강제 개방, 입력 설정 저장 등) | 45분 | 남은 일만 남는다 |
  | 3 | **`net.c` 나누기** (1,917줄) — 세션·전송 / 설정 명령 / 사용자 명령 / 이벤트 명령. 기능 변화 없이 | 2시간 | `run_tests.sh` 전부 통과 + 보드 배포 |
  | 4 | **사용자 조회 명령 4종** — `UserCountCheck` · `UserRegisterCheck` · `UserListReceive` · `UserReset` (SDK 프레임) | 2시간 | 시험 통과 + 실제 DM 1회 |
  | — | 여유 | 2시간 15분 | |

  🙋 사용자: DM에서 ncu01 **사용자 개수 조회 / 등록 여부 확인** 한 번씩

  ---

  **9/17 (목)** — 시각과 강제 개방, webui

  | # | 작업 | 시간 | 끝 기준 |
  |---|---|---:|---|
  | 1 | 아침 점검 + DM 회신 반영 | 1시간 | |
  | 2 | **`0xCE` 강제 개방을 리더별로** — 실제 트래픽에서 리더마다 4번 온다. 지금은 컨트롤러 값 하나로 답한다 | 1시간 | 리더별 설정·조회, 실제 DM 1회 |
  | 3 | **시계가 맞기 전 이벤트** — RTC가 죽어 있어(I2C 풀업 누락) 전원을 넣으면 DM 시각을 받기 전까지 시계가 틀리다. 그동안 생긴 이벤트 시각을 어떻게 할지 결정·구현 | 1.5시간 | 부팅 직후 카드 태그 → DM에 올라간 시각이 맞다 |
  | 4 | **webui 서비스화** — 네트워크 설정이 webui에 있어 없으면 현장에서 IP를 못 바꾼다. 보드에 올리고 비밀키 기본값·인증 정리 | 2시간 | 보드 재부팅 후 브라우저로 접속·설정 변경 |
  | — | 여유 | 2.5시간 | |

  ---

  **9/18 (금)** — 업데이트 수신부 착수, 주간 정리

  | # | 작업 | 시간 | 끝 기준 |
  |---|---|---:|---|
  | 1 | 아침 점검 + DM 회신 반영 | 1시간 | |
  | 2 | **펌웨어 업데이트 1단계: 받기만** — 규약 파일 전송 `0xDD`/`0xDE` 수신 → 임시 파일 → sha256 확인 → `18010506`/`18010507`.<br>설치·되돌림은 결정 8건(`update/README.md`) 뒤로 — **받는 부분은 결정과 무관**하다 | 3시간 | 시험 도구로 파일을 보내면 해시 확인까지, 틀린 해시는 Failed 이벤트 |
  | 3 | **주간 정리** — README(2,217줄 진행 기록)를 요약·분리, 다음 주(추석 주) 계획, push | 1.5시간 | |
  | — | 여유 | 2.5시간 | |

  ---

  **9/15 실제 DM 사용자 확인에서 새로 나온 일** (수요일 여유 시간에 넣는다)
  - **유효기간 `expired_date` 판정** — 저장만 하고 판정에 안 쓴다. BCD `YYMMDD`로 보임 (DM에 형식 확인) · 30분
  - **시간대·유효기간 그룹을 DM에서 받기** — 지금 표는 1단계 더미. DM이 보내는 Object부터 캡처로 확인 · 1~2시간
  - **등록 사용자 거부 이벤트의 ID** — 지금은 카드번호를 싣는다 (Platinum에 카드번호만, 이름·아이디 없음). SSC-324 실측 확인 뒤 결정 · 20분

  **이번 주에 넣지 않은 큰 줄기** (하드웨어·결정이 끼어 속도가 다르다 — 착수 때 따로 잡는다)
  - **OS 자체 빌드(A/B)** — 유저랜드 A/B/C 택일 대기
  - **RRU 연동(6단계)** — 개발보드 도착 + RRU 쪽 회신 8건 대기
  ...
  **9/15 오전까지 끝낸 것** (상세는 README 9/15 절)
  ...
  - 실제 DM: 「장치 정보 업데이트」 180초 → 17초, 비트맵 해석 확정, **입력·출력 쓰기 + 설정 성공 이벤트 DM 화면 확인**
  - ⏸ 카드리더 쓰기 0byte — DM 질의 `dm/NCU_질의_설정쓰기_실측_20260915.md`

  **9/21 주는 작업 거의 없음** — 9/24(목)부터 추석

- **🆕 NCU·RRU 원격 펌웨어 업데이트 (2026-09-14 사용자 요청)** — `update/` 문서함 신설
  - **기존 IDTi 장비에도 DM에도 없는 기능이다.** 그래서 상대가 정해져 있지 않다 —
    DM에 붙일 수도, 별도 프로그램으로 만들 수도 있다
  - 🟢 **새 프로토콜을 만들 필요가 없다**: 규약에 범용 파일 전송 `0xDD`(Start)/`0xDE`(Continue)가 있고
    (`6. FileBinaryTransmit`), `cmd ASCII` 자유 문자열에 "이 파일이 무엇인지"를 적으면 된다.
    결과 이벤트 코드도 있다 — `18010506` Success / `18010507` Failed
  - ⇒ **ACU는 보낸 쪽이 DM인지 별도 프로그램인지 구분하지 않는다.** DM 개발이 늦어도
    별도 프로그램으로 먼저 쓸 수 있고, 나중에 DM이 붙어도 ACU는 그대로다
  - 갱신 대상 셋은 성격이 다르다: **ACU 응용**(옆에 두고 바꿔 끼움) / **ACU 시스템**(A/B 파티션) /
    **RRU 펌웨어**(벽돌이 되면 함체를 열어야 한다 — 가장 위험)
  - [ ] **결정 8건 대기** (`update/README.md`) — 전달 경로, DM 파일 전송 구현 여부, 서명,
        RRU 플래시 두 벌, TYPE `0x50~0x5F` 예약, 중복 실행, 진행률 오브젝트, 시작 주체
  - 구현은 **위 결정과 RRU 개발보드 이후**다. 이번 주 일정에는 넣지 않았다

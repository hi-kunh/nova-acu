...
- **▶ 화요일(9/15) 여기서 바로 시작** (2026-09-14 월 진행 기록)
  ...
  **0. 9/14 하루에 끝낸 것 — 이번 주 일정이 비었다**
  ...
  | 9/17 DM 명령·이벤트 | ⬜ **남음** — 강제 개방 206, 화재·알람, 조회 명령(원시 바이트 대기) |

  ⇒ **화요일에 할 것**: 9/17 몫(아래 2번)을 앞당기거나, 대기 중인 회신이 오면 그것부터.
  막힌 것은 전부 **회신 대기**다 — DM 3건(`dm/README.md`), RRU 8건(`rru/README.md`).
  ...
  **2. 화요일 작업 후보: 9/17 몫 — DM 명령·이벤트** (원시 바이트 없이도 되는 것부터)
  - [ ] **강제 개방 Object `0xCE`(206)** 수신 — `SendStatus/Change`, Data Normal=0/ForcedOpen=1.
        오늘 보드 로그에 `6/2/0xCE`(읽기)도 4회 왔다 — 읽기 방향도 함께 본다
  - [ ] **화재·알람 이벤트를 mock으로 올리기** — 화재 `18010401/02`, 알람 `18010306/07`(코드는 확정)
  - [ ] **USB 단절 이벤트 골격** `0x20030102` — RRU 연결이 없어도 HAL 자리로 흉내 낼 수 있다
  - ⚠ **입력 설정 Object 44 수신**과 **알람 릴레이 규칙**은 DM 원시 바이트(특히 출력 설정)가 와야 한다

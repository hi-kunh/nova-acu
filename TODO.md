...
- [ ] **RRU MCU 선정** (미정) — **네이티브 USB 지원 여부를 선정 기준에 넣을 것**.
      있으면 CDC-ACM(`/dev/ttyACM0`), 없으면 브릿지 IC 추가(`/dev/ttyUSB0`). ACU 코드는 양쪽 동일
- [ ] J18 결선 정의 — 4포트 중 사용 포트, VBUS 연결 여부, GND 처리
      (RRU는 자체 전원 사용. 리더8+릴레이8은 USB 500mA로 감당 불가)
- [ ] udev 규칙으로 `/dev/rru` 고정 심볼릭 링크 (VID/PID 기준, MCU/브릿지 선정 후)

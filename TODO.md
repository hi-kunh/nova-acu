...
- [ ] **`InactivityTime`** — DM 기본값 **600초(10분)**. 그 시간 동안 클라이언트 활동이 없으면
      소켓을 닫는다는 의미(2026-09-10 사용자 확인). 지금은 **0으로 보고하고 동작도 미구현**이라,
      DM에서 저장한 뒤 다시 조회하면 0으로 보여 혼란스럽다
  - [ ] 값을 설정으로 두고 IMIN에 실제 값 보고
  - [ ] `net.c`에 유휴 타임아웃 구현 — 케이블만 빠진 반쯤 죽은 연결을 정리하는 데 필요하다
...
- [ ] **IMIN의 미구현 필드** — `DhcpMode`(현재 항상 0=고정), `Connect`(현재 항상 0).
      실제 상태를 반영하려면 각각 NM 설정 조회와 net.c의 연결 상태가 필요하다

...
- **▶ 화요일(9/15) 여기서 바로 시작** (2026-09-14 월 진행 기록)
  ...
  **2. 오늘 작업: 이벤트 영속 저장 `events.db`** (9/15 일정)
  - 지금 이벤트는 `net.c`의 **메모리 큐 32칸**뿐이다 — 재부팅·전원 차단이면 전부 사라지고, 가득 차면 오래된 것을 버린다
  - [ ] SQLite WAL로 `events.db`, 전송 위치(마지막으로 DM이 가져간 지점) 보관
  - [ ] **1,000만 건 링 삭제**, 한 응답 **200~500건**(DM 회신 권고, `OneDataBlockSize=36`·블록 인덱스)
  - [ ] 화재 정책으로 DM 폴링이 멈춘 동안의 이벤트 보관
  - **끝 기준**: 보드에서 재부팅·전원 차단 후 이벤트 유지, DM에 이어서 올라감
...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  | **9/15 (화)** | **이벤트 영속 저장 `events.db`** — WAL, 전송 위치, 1,000만 건 링 삭제 | 보드에서 재부팅·전원 차단 후 이벤트 유지, DM에 이어서 올라감 |

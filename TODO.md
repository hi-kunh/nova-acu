...
- [ ] **`SETT`(설정 변경) 구현** — 지금은 `FAIL`로 명시 거절한다(무응답보다 낫다)
  - [ ] `deploy/acu-netcfg`를 호출해 검증/충돌검사를 재사용할지 설계.
        acud는 `acud` 계정으로 도는데 sudoers는 `%acud` 그룹에 열려 있어 호출 자체는 가능하다
  - [ ] **자동 롤백을 쓸지 결정** — webui와 달리 탐색은 IP가 틀려도 장비를 다시 찾을 수 있으므로
        롤백이 오히려 혼란스러울 수 있다(PC는 성공했다고 보는데 되돌아감). 충돌 검사는 하되
        롤백 없이 즉시 적용하는 쪽이 맞아 보인다
  - [ ] 비밀번호 검증: Company `"IDTi"` 고정 + Custom 기본 `"0000"`

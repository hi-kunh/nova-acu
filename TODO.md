...
- **▶ 이번 주 일정 (2026-09-15 화 ~ 09-18 금)** — 9/14 저녁에 다시 세움
  ...
  **원시 바이트와 Object 번호가 전부 확정**돼, 이번 주는 **조회 명령 구현**이 중심이 된다.
  ...
  | 날짜 | 작업 | 끝났다고 보는 기준 |
  |------|------|------|
  | **9/15 (화)** | **① 헤더 `AddressDestBroadcast` 비트맵 해석** (요청이 어느 칸을 묻는지)<br>**② 조회 명령 3종** — `0x29`(41) 컨트롤러 기본 · `0x2c`(44) 입력 · `0x2d`(45) 출력 | DM 캡처와 **byte 단위로 같은 응답**. Platinum 「장치 정보 업데이트」에서 이 3종이 성공 |

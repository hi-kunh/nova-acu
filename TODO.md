...
- [ ] **물리 계층 확정: USB vs UART** — 현재 **USB 유력** (IO 보드 J18 = 2.54mm 10핀 내부 USB 헤더,
      함체 내부 보드간 연결에 적합. UART의 핀먹스/콘솔 충돌 문제가 전부 사라짐)
  - [ ] USB면: RRU 쪽 네이티브 CDC-ACM vs 브릿지 IC(CP2102/FT232) 선택
  - [ ] USB면: udev 규칙으로 `/dev/rru` 고정 심볼릭 링크
  - [ ] USB면: 재열거 대비 — RRU 이벤트 버퍼 + ACK, ACU 재연결 로직
  - [ ] UART면: 실보드 `ls /boot/dtbo/ | grep -i uart` 로 콘솔 아닌 포트 확보

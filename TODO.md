...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  | ~~**9/16 (수)**~~ ✅ | **사용자 저장 `users.db`** + **바이너리 전송 수신** | **9/14에 앞당겨 완료** — 보드 26만 명 10.25초 / 조회 0.0788ms / 10만 명 다운로드 5.4초.<br>남은 것: **1명씩 경로**(`UserSend`·`UserReceive`·`UserDelete`·`UserRegisterCheck`)와 **읽기 방향**(`UserBinTransReceive`) |

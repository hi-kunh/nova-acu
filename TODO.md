...
- **▶ 이번 주 일정 (2026-09-15 화 ~ 09-18 금)** — 9/14 저녁에 다시 세움
  ...
  **원시 바이트와 Object 번호가 전부 확정**돼, 이번 주는 **조회 명령 구현**이 중심이 된다.
  ...
  | **9/18 (금)** | **⑤ 남은 조회 명령** — `0x2b`(43) 컨트롤러 장치 · `0x2f`(47) 카드리더 · `0x51~0x54` 알람/스케줄 3종<br>+ 주간 정리·push | Platinum 「장치 정보 업데이트」가 **실패 0건**(엑세스그룹 제외)<br>⏸ **DM 캡처 대기** — `0x2b`·`0x2f`는 DM이 보내기로 함. 알람·스케줄은 **규약 문서에 형식이 없다**(`4.` 문서는 타임존·휴일·유효기간뿐).<br>SDK(`clsDevDeviceSetting.cs`·`clsDevDeviceSchedule.cs`)에 byte 배치는 있으나 **어느 칸을 지정하는지(주소 방식)** 를 모른다 |

  **9/15 진행 결과 (화)** — 화·수·목 몫을 하루에 끝냈다. **보드가 꺼져 있어 전부 개발 PC 검증까지**다.
  - [ ] **보드 배포 후 실제 DM으로 확인** — Platinum 「장치 정보 업데이트」: `0x29` 성공, `0x2C`·`0x2D`는 **DM이 설정을 내려보내기 전까지 Fail이 정상**

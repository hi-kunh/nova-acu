...
- **▶ 월요일(9/14) 여기서 바로 시작** (2026-09-11 금 마감 기록)
  ...
  **1. 아침 점검 (10분)**
  ...
  - [ ] `git pull` 후 저장소 루트에 **DM 회신 파일**이 왔는지 확인 (`ls *회신*`, `git status`) —
        보낸 질의서는 `NCU_DM_질의_RRU구성_이벤트보관.md`(질의 1~7). **왔으면 이것부터 반영**
  - [ ] DM에서 설정 가져오기를 다시 했는지 — 모듈 2개(리더 4 · 입력 16 · 출력 8)로 보이는지
...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  - DM 회신에 따라 바뀌는 것: 입력 설정 수신(질의 5), 화재 주소(6-1), 이벤트 여러 건 응답(4-1), 사용자 한도(7)
...
- **이전 기록 — 다음 시작 지점** (2026-09-10~11 진행분, 참고용)
  ...
  1-1. **DM 질의서 `NCU_DM_질의_RRU구성_이벤트보관.md` 전달 (2026-09-11) — 회신 대기**.
...
> - [ ] DM 쪽 확인: 번호를 건너뛰어 모듈 번호가 띄어져 와도(예: RRU 1·3 → 모듈 1~2, 5~6) 포인트 생성·이벤트 표시에 문제없는지

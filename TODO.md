...
- **다음 주 일정 (2026-09-14 월 ~ 09-18 금)** — RRU 개발보드 없이 할 수 있는 acud 기능부터. DM 회신이 오면 그날 먼저 반영
  ...
  - 생체 데이터: 지금 지문 미사용. 추후 지문·안면을 DM에서 받아 생체망으로 전달 — 보관 여부는 장치 선정 후
    (HARDWARE.md "네트워크 2개 분리"). 9/16 사용자 DB는 생체 데이터를 나중에 붙일 수 있게 표를 나눠 둔다
...
- [ ] **네트워크 2포트 + MAC IC 2개 (2026-09-11 결정)** — CM3는 GMAC 1개뿐이라 두 번째 포트는 PCIe NIC 추천.
      MAC은 EUI-48 I2C EEPROM을 DT `nvmem-cells`로 연결. 인터페이스 이름 고정(`dm0`/`bio0`), 망 사이 전달 금지,
      acud 리슨을 DM망에만, IP 설정 두 벌(`acu-netcfg` 확장). HARDWARE.md "네트워크 2개 분리" 절

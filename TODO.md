...
- **▶ 이번 주 일정 (2026-09-15 화 ~ 09-18 금)** — 9/14 저녁에 다시 세움

  9/15~9/18 예정이던 작업은 **9/14에 모두 끝냈다.** 그날 저녁 DM 회신(`dm/DM_회신_Object번호확정_원시바이트_20260914.md`)으로
  **원시 바이트와 Object 번호가 전부 확정**돼, 이번 주는 **조회 명령 구현**이 중심이 된다.

  | 날짜 | 작업 | 끝났다고 보는 기준 |
  |------|------|------|
  | **9/15 (화)** ✅ | **① 헤더 `AddressDestBroadcast` 비트맵 해석** (요청이 어느 칸을 묻는지)<br>**② 조회 명령 3종** — `0x29`(41) 컨트롤러 기본 · `0x2c`(44) 입력 · `0x2d`(45) 출력 | ✅ **캡처와 일치** (`tools/setting_read_test.py`). 비트맵은 추정(칸 번호 = 비트) — DM 대응표 대기.<br>⬜ 보드 배포 후 Platinum 「장치 정보 업데이트」 확인 (0x29는 성공, 0x2C·0x2D는 **설정이 저장되기 전까지 Fail이 정상**) |
  | **9/16 (수)** ✅ | **③ 입력·출력 설정 저장**(쓰기 `Sub 5` 포함) + **알람 릴레이 규칙**<br>`ActiveType=2(Alarm)` 출력 전부 + `IsActiveOption` bit7(화재로 동작) | ✅ **9/15에 앞당겨 완료** (`tools/setting_write_test.py`). 설정 성공 이벤트, 알람·화재 겹침까지 확인.<br>⚠ "화재로도 함께"를 Alarm 출력 중 bit7로 읽음 — DM 확인 |
  | **9/17 (목)** ✅ | **④ 이벤트 인덱스 3종** — `EventCountCheck` · `EventIndexChange` · `EventReset`<br>(DM 회신 6절: 현장 SSC-324가 이것 때문에 이벤트를 못 올린 고장이 있었다) | ✅ **9/15에 앞당겨 완료** — 자동 복구(쓰기 직전 검사), 링 경계 시험 통과.<br>⚠ 의미는 추정(개수=미전송, 리셋=보냄 표시, 인덱스 타입 3만) — DM 실측 #3 대기 |
  | **9/18 (금)** ✅ | **⑤ 남은 조회 명령** — `0x2b`(43) 컨트롤러 장치 · `0x2f`(47) 카드리더 · `0x51~0x54` 알람/스케줄 3종 | ✅ **9/15에 앞당겨 완료** — 주소는 **실제 DM 트래픽**에서, 크기는 SDK에서(캡처 3개와 일치 확인). `tools/setting_all_test.py`.<br>✅ 실제 DM 3차 확인: 53건 16.9초, **주소가 틀린 요청 0건**, 미지원은 `0xA6` 4건만 |

  **9/15 진행 결과 (화)** — **화·수·목·금 몫을 하루에 끝냈다.** 보드 배포 + 실제 DM으로 두 번 확인했다.
  - ✅ 실제 DM 「장치 정보 업데이트」: 1차 180초 → 미지원 실패 ACK 뒤 2차 **17초**. 비트맵 해석이 입력·출력·리더 전 칸에서 확정
  - ✅ 미지원 요청 로그의 주소로 금요일 몫 단위를 읽어 내고 6종 구현
  - ✅ 실제 DM 3차(6종 배포 뒤): 53건 16.9초 — 성공 5(`0x29`·`0xCE`×4) / **저장 없음 Fail 44** / 미지원 4(`0xA6`) / **주소 틀림 0**
  - ✅ **실제 DM 쓰기**: 입력(모듈1 칸3)·출력(모듈1 칸9) 저장 + `10210301`/`10220301` 올라감, DM이 가져감
  - [ ] 🔴 **카드리더 쓰기는 DM이 데이터 0byte로 보낸다** (모듈1 비트맵 bit0, 주소는 맞음) — acud는 Fail로 답한다(거짓 성공 금지).
        질의 `dm/NCU_질의_설정쓰기_실측_20260915.md` 3절: 정상인지 / SSC-324에는 몇 byte인지 / SSC-324는 무엇으로 답하는지
  - [x] ~~DM 화면에 입력·출력 설정 성공 이벤트가 보였는지~~ — **9/15 사용자 확인: 올라왔다.** 쓰기 경로가 DM 화면까지 검증됨
  - [ ] `0xA6` 그룹 적용(요청에 데이터 9byte) — SSC-324도 실패 중이라 가장 뒤
  - [x] ~~보드 배포 후 실제 DM으로 확인~~ — `0x29` 성공, 입력·출력은 저장 없음 Fail(정상) 확인
  - [ ] DM에서 입력·출력 설정을 한 칸 내려보내 → `10210301`/`10220301`이 DM 화면에 뜨는지, 되읽기가 성공하는지
  - [ ] DM에 새로 물을 것: 설정 조회 **Fail 응답의 원시 바이트**(엑세스그룹 `0xa6`가 SSC-324에서도 실패하니 그걸로 뜰 수 있다) ·
        알람·스케줄 **주소 방식** · 응답 주소·FrameOption·Password 칸의 의미 · FrameOption `IsEventAgain`(bit5, 직전 이벤트 재요청)을 DM이 쓰는지

  **매일 아침**: 보드 전원·`systemctl is-active acud` → `ls dm/ rru/`로 새 회신 확인 → 그날 작업

  **회신이 오면 그날 먼저 반영한다**
  - DM: 4·5순위 원시 바이트(`0x2b`·`0x2f`), **비트맵 ↔ 포인트 번호 대응표**, 성공 ACK 조사 결과, 쓰기 계열 재검증
  - RRU: 프로토콜 초안 회신 8건 (`rru/README.md`)
  - 업데이트: 설계 초안 결정 8건 (`update/README.md`)

  ⚠ **비트맵 대응표가 오기 전에는 ②를 "1번 칸만" 검증할 수밖에 없다.** DM이 입력 1~16을 훑어
  표로 만들어 주기로 했으니, 오면 그때 전 칸을 맞춘다.

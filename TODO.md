...
- [x] ~~RRU 대수~~ → **1대 고정**, ~~물리 규격~~ → **RS-232 point-to-point** (확정)
  - 결과: 프레임에 **장치 주소 필드 불필요**, 카드 이벤트는 **RRU 능동 송신**(full duplex라 폴링 불필요)
- [x] ~~baud/프레이밍~~ → **115200 8N1** (확정)
- [ ] **UART 포트 확정** — 실보드에서 `ls /boot/dtbo/ | grep -i uart` 로 사용 가능한 오버레이 확인.
      콘솔이 아닌 UART 우선, 없으면 UART2(40핀 8/10번) + 콘솔 비활성화 (HARDWARE.md 참고)
- [ ] **RS-232 트랜시버 준비 or TTL 직결 결정** — 40핀 헤더는 3.3V TTL, IO 보드에 트랜시버 없음
- [ ] ACU↔RRU 배선 거리 확인 — RS-232 실용 한계(약 15m) 안인지
...
- [ ] config.json에 UART 설정 추가 (포트 경로, baud) + 웹 UI 반영

...
- [ ] **1. 화재·알람 이벤트를 어느 모듈 주소로 올릴지** — (A) 실제로 물린 모듈 하나만 / (B) 모듈 1~4 동시.
...
별도 RRU 보드(리더8 / 입력24 / 출력16)와 USB로 통신**한다. 구성 상세는 [HARDWARE.md](HARDWARE.md).
...
- [ ] **최종 RRU 보드 I/O 설계 (2모듈 검증 완료 후)** — LQFP64(`RE`)로는 56 I/O(Wiegand16+입력24+출력16)가
      한계 초과. MCU는 STM32C562로 고정하되 **패키지는 재검토**: 계열 내 상위 핀수 옵션 확인,
      느린 입력24/출력16은 I/O 익스팬더로 분리(단, Wiegand D0/D1에는 익스팬더 금지),
      16개 Wiegand 핀 배치 확정

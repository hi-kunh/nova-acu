...
- **다음 시작 지점** (2026-09-10 마감 기준)
  1. **Platinum 쪽 답 받기** — 이벤트 코드 확인 요청 (아래 "설정 소유권 설계" 앞 D절 + HARDWARE.md "제품 I/O 구성"의 코드표)
     - **알람 코드 재검토** — `0x18010306/07`은 화면에 "모션 알람"으로 떠 부자연스럽다
     - 화재·알람 코드가 Platinum 화면에 제대로 뜨는지 (`eventcode_tbl`의 `StringCode` 연결)
     - **USB 단절·복구에 쓸 기존 코드** — 새 코드는 만들 수 없다(기존 IDTi 보드 대체). 기존 보드가
       SSC-R4(I/O 보드) 연결 끊김에 쓰던 코드가 있으면 그것을 쓴다
  2. 답을 받으면 구현(사용자 승인 후): DM 강제 개방 명령 수신(Object 206), 화재·알람 이벤트, USB 단절·복구 이벤트
...
| 이벤트 코드 | 화재 `0x18010401/02` 확정. **알람은 재검토, USB 단절·복구는 기존 코드에서 선택** — Platinum 확인 대기 |
...
- [x] ~~**1. 화재·알람 이벤트를 어느 모듈 주소로 올릴지**~~ -> **질문의 전제가 틀렸다** (2026-09-10 SSC-324 실물 확인).
  ...
  - [ ] **화재·알람 입력 이벤트를 DM에 올리기** — 화재 `0x18010401`/`0x18010402`,
        알람 코드는 **재검토 중** (HARDWARE.md 코드표의 후보 참고)
  ...
  - [ ] **알람 입력 코드 재검토** — `0x18010306/07`(Intrusion Motion)로 정했다가 Platinum 화면 문구가
        "모션 알람 발생/복구"라 다시 보기로 함 (2026-09-10). 후보 `0x18010304/05` Intrusion, `0x18010111/12` Sensor.
        Platinum 쪽 확인
  - [ ] **USB 단절·복구 코드** — **새 코드는 만들 수 없다(기존 IDTi 보드 대체). 기존 코드에서 고른다.**

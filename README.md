...
- ⚠ **카드는 앞 8byte가 뒤집혀 실린다.** `clsDevUserBin.cs`에 그대로 적혀 있다:
  *"실제 사용 바이트는 8 바이트 이고, 단말기의 바이너리 전송 처리를 위해 카드정보 송/수신시
  꼬았던 바이트를 원래대로 복구시켜서 보내야 함"* — 코드도 `Copy(8) → Reverse → Prox32[0..7]`이다.
  되돌려야 단일 사용자 경로(Proximity Data 32byte)의 카드값과 같아진다

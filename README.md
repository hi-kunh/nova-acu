...
### 2026-09-11 — 네트워크 2개 분리 · MAC 주소 IC 2개 결정

지금 현장 IDTi는 지문을 쓰지 않지만, 추후 **지문·안면 인식**을 DM에서 받아 NCU가 장치로 전달한다.
그래서 **DM망과 생체 인식 장치망을 물리적으로 분리**하고, **MAC 주소 IC를 포트마다 1개씩 2개** 보드에 넣기로 했다.
...
- 소프트웨어: 인터페이스 이름 고정, 망 사이 전달 금지, acud 리슨을 DM망에만, IP 설정 두 벌

상세는 HARDWARE.md "네트워크 2개 분리 · MAC 주소 IC 2개" 절.

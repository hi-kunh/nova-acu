...
**RRU 여러 대 구성** — RRU를 모듈2(리더 4) / 모듈4(리더 8) 두 종류로 만들고, 한 ACU에 여러 대(16대 정도 요구)를
같은 함체 안에서 USB로 붙인다. RRU 구분은 DIP 스위치 ID. **프로토콜 V2의 모듈 14개 한계**(기존 장비 최대 리더 28과 같다)와
부딪혀 최대 대수는 결정 대기다. RRU는 **모델명**을 알려 주고 ACU는 모델명으로 모듈 구성을 안다.

...
```
[상위 시스템] --TCP/IDTi V2--> [ACU: RK3566] --USB--> [RRU: 리더8 / 입력24 / 출력16]
```
...
- 기존 제품 매뉴얼 — `1Reader per 3-Sensor(Input), 2-Relay(Output)`, 보드 공통 `SEN0_1(ALARM)`/`SEN0_2(FIRE)`
...
모듈 1~4 = mType 1 + InstallType 1 + 슬롯 14 = "22333333444433"
           카드리더 2 · 입력 6 · 출력 4 · 공통입력 2(화재·알람)
합계: 리더 8 · 입력 32 · 출력 16
```

- 모듈 하나의 모양이 **현장 SSC-324와 한 글자도 같다** — 서버·관리프로그램 쪽은 고칠 것이 없다
- 공통 입력(화재·알람)은 물리적으로 2개인데 프로토콜에 "보드 공통" 개념이 없어
  **모듈마다 실린다 → 서버에는 입력 32개(24 + 2×4)로 잡힌다.** SSC-324도 같은 방식이다
- 처음엔 타입별로 8개씩 묶어 모듈 6개로 보냈다(서버는 이것도 받아 리더8·입력24·출력16을 만들었다).
  하지만 공통 입력 자리가 없고 모듈 모양이 SSC-324와 달라 대치 목적에 맞지 않아 바꿨다

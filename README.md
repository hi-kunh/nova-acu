...
- **DM 쪽이 보내 준 규약 문서 `NCU_장치구조_프로토콜_안내.md`** — 현장 SSC-324(ACU03)의 실제 응답과 서버 DB 기준
...
**Platinum 회신** (`NCU_이벤트코드_질의_회신.md`, 전체 코드표 `event_codes_all.csv` 987개)
...
**기존 장비 사용자 용량은 10만이 아니라 13만 / 26만이다.** 매뉴얼 `USER : 130,000 / 260,000`(SSC-332 / SSC-334)이고,
기존 장비 SD 카드 이미지(`SSC-334.bin`)의 사용자 파일도 262,144칸으로 미리 잡혀 있다
(`GAPB_USR.bin` = 5byte × 262,144, `b_restri.bin` = 160byte × 262,144). 현장 SSC-324 펌웨어의 실제 한도는 DM 쪽에 질의했다.
